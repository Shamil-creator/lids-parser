...
class Messenger:
    ...
    async def send_first_message(self, user_id: int, username: str = "", channel_source: str = "", original_post_text: str = "", force_repeat: bool = False) -> bool:
        ...
        try:
            ...
            # Сохраняем информацию о пользователе (original_post_text, channel_source) без пометки как обработанного
            # Это нужно, чтобы при ответе пользователя можно было показать полный исходный пост менеджерам
            if original_post_text or channel_source:
                db.save_user_info(user_id, username, channel_source, original_post_text)
                print(f"[{self.session_name}] 💾 Информация о пользователе сохранена (original_post_text сохранен)")
    ...
    async def forward_message_to_managers(self, message: Message, source_channel: str = "", original_post_text: str = "", parser=None):
        ...
        try:
            ...
            # Показываем полный исходный пост (если он длинный, Telegram сам ограничит при отправке)
            original_post_display = original_post_text if original_post_text else 'Не указан'
...
"""
    ...
    async def forward_lead_to_managers(self, user_id: int, username: str, phone: str, 
        ...
        try:
            # Формирование сообщения для менеджеров
            # Показываем полный исходный пост
            original_post_display = original_post_text if original_post_text else 'Не указан'
